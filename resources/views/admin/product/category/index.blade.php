                            ...
                            <div class="table-responsive">
                                <table id="datatable" class="table table-hover mb-0">
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            @if( $data -> status == 'Published' )
                                                <a href="{{ route('shop-category.unpublished', $data -> id) }}" class="btn btn-danger btn-sm"><i class="fas fa-eye-slash"></i></a>
                                            @else
                                                <a href="{{ route('shop-category.published', $data -> id) }}" class="btn btn-success btn-sm"><i class="fas fa-eye"></i></a>
                                            @endif
                                            <a id="edit_product_category" edit_id="{{ $data -> id }}" class="btn btn-warning btn-sm" href="#" data-toggle="modal">Edit</a>
                                                <form class="d-inline" action="{{ route('shop-category.delete', $data -> id) }}" method="POST">
                                                    @csrf
                                                    @method('DELETE')
            ...
            <div id="category-edit-modal" class="modal fade">
                <div class="modal-dialog modal-dialog-centered">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <form action="{{ route('shop-category.update') }}" method="POST">
                                @csrf
                                @method('PUT')
                                <div class="form-group">
                                    <input name="name" class="form-control" type="text" placeholder="Name">
